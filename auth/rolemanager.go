...
func (l *eventLoggingRoleManager) DeleteRole(role string) error {
	...
	return event.LogRoleEvent(event.RoleEventTypeRemove, &roles[0], nil)
}
...
func (l *eventLoggingRoleManager) UpdateRole(role gimlet.Role) error {
	...
	eventType := event.RoleEventTypeAdd
	var before *gimlet.Role
	if len(roles) > 0 {
		before = &roles[0]
		eventType = event.RoleEventTypeUpdate
	}
	return event.LogRoleEvent(eventType, before, &role)
}
...
func (l *eventLoggingRoleManager) Clear() error {
	...
	for _, role := range roles {
		catcher.Add(event.LogRoleEvent(event.RoleEventTypeRemove, &role, nil))
	}
